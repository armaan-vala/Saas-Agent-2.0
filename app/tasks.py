...
def process_document(document_id, agent_id):
    """
    ...
    """
    print(f"Starting REAL processing for document ID: {document_id} for Agent ID: {agent_id}")
    ...
    try:
        ...
        agent_vector_store_path = os.path.join(VECTOR_STORE_DIR, f"agent_{agent_id}") 
        ...
        doc.status = 'Complete'
        db.session.commit()
        print(f"SUCCESS: Document {doc.filename} has been processed and embedded for agent {agent_id}.")
        return f"Document {doc.id} embedded successfully for agent {agent_id}."

    except Exception as e:
        print(f"ERROR processing document {doc.id} for agent {agent_id}: {str(e)}")
        doc.status = 'Error'
        db.session.commit()
        return f"Error processing document {doc.id} for agent {agent_id}."